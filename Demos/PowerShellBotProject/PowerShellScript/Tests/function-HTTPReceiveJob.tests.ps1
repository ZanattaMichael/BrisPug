...
Describe "HTTPReceiveJob.ps1 Tests" {
        ...
        Context "Testing Standard Request - Should not be null" {
            ...
            BeforeAll {
                ...
                $Body = @{
                    GUID = bf28c01f-9f8c-47c1-ae30-f1e310930e00
                    ComputerName = "TEST01"
        ...
        Context "Testing Standard Request - Sending Invalid Data" {
            ...
            it "Testing Missing Code Property - Looking for Error Property" {                
                # Set the Body (Missing the Status Property)
                $Body = @{
                    GUID = bf28c01f-9f8c-47c1-ae30-f1e310930e00
                    ComputerName = "TEST01"
                } | ConvertTo-Json
            ...
            it "Testing Missing ComputerName Property - Looking for Error Property" {                
                # Set the Body (Missing the ComputerName Property)
                $Body = @{
                    GUID = bf28c01f-9f8c-47c1-ae30-f1e310930e00
                    Status = [System.Net.HttpStatusCode]::OK
                } | ConvertTo-Json
            ...
            # Test the Response. Looking for Nulls
            it "Testing Missing Code Property - Looking for Error Property" {                
                # Set the Body (Missing the GUID Property)
                $Body = @{
                    ComputerName = "TEST01"
                    Status = [System.Net.HttpStatusCode]::OK
                } | ConvertTo-Json
